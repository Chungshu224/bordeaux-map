...
const shareGame = async () => {
  ...
  try {
    ...
    if (navigator.canShare?.({ files: [file] })) {
      await navigator.share({ title: '年份排序挑戰', text: `我在侍酒師的筆記本拿到 ${score.value} 分！`, files: [file] })
    } else {
...
function nextRound() {
  if (roundIdx.value < 4) {
    ...
    const _prev = parseInt(localStorage.getItem('bdx_vintage_best') || '0')
    if (score.value > _prev) localStorage.setItem('bdx_vintage_best', score.value)
    phase.value = 'final'
  }
}
