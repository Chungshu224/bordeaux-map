<template>
  <div class="alsace-slide-viewer">
    <!-- Header -->
    <header class="sv-header">
      <button class="sv-back" @click="emit('close')">← 返回列表</button>
    ...
    <nav class="sv-nav-row">
      ...
      <button v-if="currentSlide < slides.length - 1" class="sv-nav-btn primary" @click="nextSlide">下一頁 →</button>
      <button v-else class="sv-nav-btn complete" @click="complete">✓ 完成課程</button>
    </nav>
    ...
    <div class="sv-content">
      <div class="slide-container">
        ...
        <template v-if="currentSlideData.type === 'cover'">
          <div class="slide-cover" :style="{ background: currentSlideData.gradient || 'linear-gradient(135deg,#2d6a4f,#52b788)' }">
        ...
        <template v-else-if="currentSlideData.type === 'summary'">
          <div class="slide-summary-card">
            <div class="summary-icon">{{ currentSlideData.icon || '🎉' }}</div>
            <h2 class="summary-title">課程完成！</h2>
            <p class="summary-desc">{{ currentSlideData.message || '您已完成本課程，繼續保持！' }}</p>
            <div v-if="currentSlideData.keyPoints && currentSlideData.keyPoints.length" class="summary-points">
              <div class="sp-title">📌 本課重點回顧</div>
              <ul>
                <li v-for="pt in currentSlideData.keyPoints" :key="pt">{{ pt }}</li>
              </ul>
            </div>
            <p v-if="currentSlideData.nextLesson" class="summary-next">➡️ {{ currentSlideData.nextLesson }}</p>
          </div>
        </template>
...
import { getAlsaceLessonSlides } from '../data/lessonSlides.js'
...
// 取得該課程的投影片
const slides = computed(() => getAlsaceLessonSlides(props.lesson.id))
...
.sv-progress-fill {
  height: 100%;
  background: linear-gradient(90deg, #2d6a4f, #c9a227);
...
.sc-header {
  ...
  border-bottom: 2px solid #eaf5ef;
  background: #f5faf6;
}
...
.quiz-btn:hover:not(.disabled) { border-color: #2d6a4f; background: #f5faf6; }
...
.sp-title { font-weight: 700; color: #2d6a4f; margin-bottom: 10px; font-size: 0.9rem; }
...
.summary-next { font-size: 0.85rem; color: #c9a227; font-weight: 700; margin: 16px 0 0; }
...
.sv-nav-btn:hover:not(:disabled) { border-color: #2d6a4f; color: #2d6a4f; }
...
.sv-nav-btn.primary:hover { background: #1b4332; border-color: #1b4332; }
.sv-nav-btn.complete {
...
.sv-dot.active { background: #2d6a4f; }
