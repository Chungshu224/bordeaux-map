<template>
  <div class="alsace-course-layout">
    <!-- Header -->
    ...
    <div class="layout-body">
      ...
      <div class="main-area">
        <!-- Level 標題卡 -->
        <div class="level-header-card" :style="{ background: levelDef?.gradient || 'linear-gradient(135deg,#2d6a4f,#52b788)' }">
          <div class="lhc-top">
    ...
    <!-- 手機章節抽屜 -->
    <Teleport to="body">
      <div v-if="drawerOpen" class="alsace-drawer-overlay" @click="drawerOpen = false"></div>
      <Transition name="slide-up">
        <div v-if="drawerOpen" class="alsace-chapter-drawer">
...
.level-tab.active { background: white; color: #2d6a4f; }
...
.sidebar-item:hover { background: #f5faf6; }
.sidebar-icon {
  color: #2d6a4f;
  font-size: 0.85rem;
  font-weight: 700;
...
.lesson-card:hover { border-color: #52b788; background: #f5faf6; }
.lesson-card.completed { background: #f0fff4; border-color: #c8e6c9; }
.lesson-card.exam { border-style: dashed; border-color: #c9a227; }
.lesson-num {
  width: 28px;
  height: 28px;
  border-radius: 50%;
...
/* ── Drawer ──────────────────────────── */
.alsace-drawer-overlay {
  position: fixed; inset: 0;
  background: rgba(0,0,0,0.4);
  z-index: 1000;
}
.alsace-chapter-drawer {
  position: fixed;
...
.drawer-item:hover { background: #f5faf6; }
.drawer-status {
  color: #2d6a4f;
  font-weight: 700;
  width: 20px;
