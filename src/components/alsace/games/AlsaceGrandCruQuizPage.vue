<template>
  <div ref="pageEl" class="agq-page" tabindex="-1" @keydown="onKey">
    <button class="back-btn" @click="handleBack">← 返回</button>

    <!-- ══ LOBBY ══ -->
    <div v-if="phase === 'lobby'" class="lobby">
      <div class="lobby-inner">
        <h1 class="title">🏔️ Grand Cru 地質配對</h1>
        <p class="subtitle">51 個特級園 × Claude Sittler 十大地質族群</p>
        ...
        <div class="mode-info">
          <div class="mode-box easy-box">
            ...
            <div>
              ...
              <div class="mode-detail">Grand Cru 名稱 → 所屬地質族群</div>
            </div>
          </div>
          <div class="mode-box hard-box">
            ...
            <div>
              ...
              <div class="mode-detail">Grand Cru 名稱 → 所在村莊</div>
            </div>
          </div>
        </div>
        ...
        <div class="diff-cards">
          <div class="diff-card easy" @click="startGame('easy')">
            ...
            <div class="diff-desc">地質族群辨識</div>
            <ul class="diff-list">
              <li>4 個選項</li>
              <li>⏱ 每題 15 秒</li>
              <li>共 12 題</li>
          ...
          <div class="diff-card hard" @click="startGame('hard')">
            ...
            <div class="diff-desc">村莊歸屬辨識</div>
            <ul class="diff-list">
              <li>4 個選項</li>
              <li>⏱ 每題 12 秒</li>
              <li>共 12 題</li>
    ...
    <div v-else-if="phase === 'playing' || phase === 'feedback'" class="game">
      ...
      <div class="question-card" :class="fbCardCls">
        <div class="q-mode-badge" :class="difficulty">
          {{ difficulty === 'easy' ? '🪨 地質族群' : '🏘️ 所在村莊' }}
        </div>
        <div class="cru-name">{{ currentQ?.subject }}</div>
...
// ── 地質族群（10 大族群，Kaefferkopf 為混釀特例不列入）──────────────
const FAMILY_OPTIONS = ['片岩', '火山沉積岩', '砂岩', '花崗岩', '石灰岩', '灰砂岩', '泥灰砂岩', '泥灰石灰砂岩', '泥灰石灰岩', '黏土泥灰岩']

const EASY_BANK = [
  { subject: 'Kastelberg',             answer: '片岩' },
  { subject: 'Rangen',                 answer: '火山沉積岩' },
  { subject: 'Muenchberg',             answer: '火山沉積岩' },
  { subject: 'Kessler',                answer: '砂岩' },
  { subject: 'Kitterlé',               answer: '砂岩' },
  { subject: 'Schlossberg',            answer: '花崗岩' },
  { subject: 'Brand',                  answer: '花崗岩' },
  { subject: 'Sommerberg',             answer: '花崗岩' },
  { subject: 'Steinklotz',             answer: '石灰岩' },
  { subject: 'Rosacker',               answer: '石灰岩' },
  { subject: 'Zinnkoepflé',            answer: '灰砂岩' },
  { subject: 'Geisberg',               answer: '灰砂岩' },
  { subject: 'Pfersigberg',            answer: '泥灰砂岩' },
  { subject: 'Vorbourg',               answer: '泥灰砂岩' },
  { subject: 'Altenberg de Bergheim',  answer: '泥灰石灰砂岩' },
  { subject: 'Zotzenberg',             answer: '泥灰石灰砂岩' },
  { subject: 'Hengst',                 answer: '泥灰石灰岩' },
  { subject: 'Eichberg',               answer: '泥灰石灰岩' },
  { subject: 'Schoenenbourg',          answer: '黏土泥灰岩' },
  { subject: 'Kanzlerberg',            answer: '黏土泥灰岩' },
]

const COMMUNE_OPTIONS = ['Andlau', 'Thann', 'Guebwiller', 'Kientzheim', 'Turckheim', 'Marlenheim', 'Ribeauvillé', 'Soultzmatt', 'Eguisheim', 'Wintzenheim', 'Bergheim', 'Riquewihr']

const HARD_BANK = [
  { subject: 'Kastelberg',    answer: 'Andlau' },
  { subject: 'Wiebelsberg',   answer: 'Andlau' },
  { subject: 'Rangen',        answer: 'Thann' },
  { subject: 'Kessler',       answer: 'Guebwiller' },
  { subject: 'Kitterlé',      answer: 'Guebwiller' },
  { subject: 'Schlossberg',   answer: 'Kientzheim' },
  { subject: 'Brand',         answer: 'Turckheim' },
  { subject: 'Steinklotz',    answer: 'Marlenheim' },
  { subject: 'Geisberg',      answer: 'Ribeauvillé' },
  { subject: 'Osterberg',     answer: 'Ribeauvillé' },
  { subject: 'Zinnkoepflé',   answer: 'Soultzmatt' },
  { subject: 'Eichberg',      answer: 'Eguisheim' },
  { subject: 'Pfersigberg',   answer: 'Eguisheim' },
  { subject: 'Hengst',        answer: 'Wintzenheim' },
  { subject: 'Altenberg de Bergheim', answer: 'Bergheim' },
  { subject: 'Kanzlerberg',   answer: 'Bergheim' },
  { subject: 'Schoenenbourg', answer: 'Riquewihr' },
  { subject: 'Sporen',        answer: 'Riquewihr' },
]
...
function startGame(diff) {
  ...
  shuffleArr(bank)
  const pool = diff === 'easy' ? FAMILY_OPTIONS : COMMUNE_OPTIONS
...
function nextQ() {
  ...
  if (qIdx.value >= questions.value.length - 1) {
    const key = `al_grandcru_${difficulty.value}_best`
...
async function loadLeaderboard(d) {
  ...
  try {
    const { data } = await supabase.from('quiz_scores')
      ...
      .eq('game_type', 'alsace_grandcru_quiz').eq('difficulty', d)
...
async function submitScore() {
  ...
  try {
    ...
    const { error } = await supabase.from('quiz_scores').insert({
      ...
      game_type: 'alsace_grandcru_quiz', difficulty: difficulty.value,
...
.agq-page {
  ...
  background: linear-gradient(160deg, #0d1f16 0%, #12231a 45%, #141420 100%);
  color: #f0f0f0;
...
.cru-name { font-size: 1.6rem; font-weight: 800; color: #fff; }
...
.opt-btn {
  ...
  border-radius: 12px; color: #f0f0f0; font-size: 0.95rem; font-weight: 700;
