      ...
      <section class="quick-nav">
        <div class="quick-nav-grid">
          <button class="nav-card game-hub" @click="emit('openGames')">
            ...
            <span class="nav-desc">品種競答・產區辨識・風格配對</span>
          </button>
    ...
    <Teleport to="body">
      <div v-if="showAchievements" class="progress-modal-overlay" @click.self="showAchievements = false">
        <div class="progress-modal">
          ...
          <div class="pm-body">
            <AchievementsDashboard course-key="california" @back="showAchievements = false" />
          </div>
        </div>
      </div>
    </Teleport>
...
.game-hub         { background: rgba(139,26,26,0.35); border-color: rgba(139,26,26,0.5); }
.achievement-card { background: rgba(201,168,76,0.25); border-color: rgba(201,168,76,0.45); }
.progress-card    { background: rgba(139,26,26,0.2); border-color: rgba(201,168,76,0.3); }
