  ...
  <div ref="pageEl" class="cq-page" tabindex="-1" @keydown="onKey">
    ...
    <div v-if="phase === 'lobby'" class="lobby">
      <div class="lobby-inner">
        <h1 class="title">🍯 匈牙利酒款 / 分級辨識</h1>
        <p class="subtitle">經典酒款 + Tokaji 殘糖等級（Aszú 3-6 Puttonyos / Eszencia）</p>
        ...
        <div class="mode-info">
          <div class="mode-box easy-box">
            ...
            <div>
              ...
              <div class="mode-detail">描述 → 匈牙利酒款</div>
            </div>
          </div>
          <div class="mode-box hard-box">
            ...
            <div>
              ...
              <div class="mode-detail">殘糖描述 → Tokaji 分級</div>
            </div>
          </div>
        </div>
        ...
        <div class="diff-cards">
          <div class="diff-card easy" @click="startGame('easy')">
            ...
            <div class="diff-desc">經典酒款辨識</div>
            <ul class="diff-list">
              <li>4 個選項</li>
              <li>⏱ 每題 15 秒</li>
              <li>共 10 題</li>
          ...
          <div class="diff-card hard" @click="startGame('hard')">
            ...
            <div class="diff-desc">Tokaji 等級辨識</div>
            <ul class="diff-list">
              <li>4 個選項</li>
              <li>⏱ 每題 12 秒</li>
              <li>共 8 題</li>
    ...
    <div v-else-if="phase === 'playing' || phase === 'feedback'" class="game">
      ...
      <div class="question-card" :class="fbCardCls">
        <div class="q-mode-badge" :class="difficulty">
          {{ difficulty === 'easy' ? '🍷 經典酒款' : '🏷️ Tokaji 等級' }}
        </div>
...
const EASY_QUESTIONS = [
  { subject: '世界三大貴腐甜酒之一，金黃琥珀色，蜂蜜杏桃風味，路易十四譽為「酒中之王」',  answer: 'Tokaji Aszú',     wrongs: ['Szamorodni', 'Eszencia', 'Egri Bikavér'] },
  { subject: '匈牙利北部 Eger 產區的紅葡萄混釀酒，曾擊退土耳其軍隊的傳說',                  answer: 'Egri Bikavér',    wrongs: ['Tokaji Aszú', 'Furmint', 'Hárslevelű'] },
  { subject: '匈牙利白葡萄品種，酸度高，是 Tokaji Aszú 主要原料',                          answer: 'Furmint',         wrongs: ['Hárslevelű', 'Sárga Muskotály', 'Kadarka'] },
  { subject: '波蘭文意為「同樣方式」，整串葡萄釀造，可干（Száraz）可甜（Édes）',           answer: 'Szamorodni',      wrongs: ['Tokaji Aszú', 'Eszencia', 'Furmint'] },
  { subject: 'Tokaji 最稀有最珍貴等級，僅由貴腐葡萄自重壓出的天然汁液釀造，殘糖極高',       answer: 'Eszencia',        wrongs: ['Tokaji Aszú', 'Szamorodni', 'Furmint'] },
  { subject: '匈牙利白葡萄品種，意為「椴樹葉」，柔和花香，常與 Furmint 混釀',              answer: 'Hárslevelű',      wrongs: ['Furmint', 'Sárga Muskotály', 'Kéknyelű'] },
  { subject: '意為「公牛血」，傳說來自 1552 年 Eger 圍城戰',                                 answer: 'Egri Bikavér',    wrongs: ['Tokaji Aszú', 'Szamorodni', 'Eszencia'] },
  { subject: '匈牙利紅葡萄品種，曾是 Bikavér 主要原料，現多被 Kékfrankos 取代',             answer: 'Kadarka',         wrongs: ['Furmint', 'Hárslevelű', 'Kékfrankos'] },
  { subject: '匈牙利第二紅葡萄品種，即奧地利的 Blaufränkisch',                              answer: 'Kékfrankos',      wrongs: ['Kadarka', 'Furmint', 'Hárslevelű'] },
  { subject: '匈牙利常見的麝香葡萄，香氣奔放，常用於 Tokaji 混釀',                          answer: 'Sárga Muskotály', wrongs: ['Furmint', 'Hárslevelű', 'Kadarka'] },
]

// ── HARD: Tokaji Puttonyos 殘糖等級 ─────────────────────────────
const HARD_QUESTIONS = [
  { subject: '殘糖 60-90 g/L 的 Tokaji Aszú（自 2013 年新法規後不再單獨標示）',              answer: '3 Puttonyos',  wrongs: ['4 Puttonyos', '5 Puttonyos', '6 Puttonyos'] },
  { subject: '殘糖 90-120 g/L 的 Tokaji Aszú（2013 年後改為最低 120 g/L）',                  answer: '4 Puttonyos',  wrongs: ['3 Puttonyos', '5 Puttonyos', '6 Puttonyos'] },
  { subject: '殘糖最低 120 g/L 的 Tokaji Aszú，現為市售 Aszú 入門等級',                       answer: '5 Puttonyos',  wrongs: ['4 Puttonyos', '6 Puttonyos', 'Aszú Eszencia'] },
  { subject: '殘糖最低 150 g/L 的 Tokaji Aszú，最高的標示 Puttonyos 等級',                    answer: '6 Puttonyos',  wrongs: ['5 Puttonyos', 'Aszú Eszencia', 'Eszencia'] },
  { subject: '殘糖最低 180 g/L 的 Tokaji，介於 6 Puttonyos 與 Eszencia 之間（舊法規）',       answer: 'Aszú Eszencia', wrongs: ['6 Puttonyos', 'Eszencia', '5 Puttonyos'] },
  { subject: '殘糖最低 450 g/L 的天然貴腐自流汁，世界最甜的葡萄酒之一',                      answer: 'Eszencia',     wrongs: ['Aszú Eszencia', '6 Puttonyos', 'Szamorodni Édes'] },
  { subject: '整串釀造的 Szamorodni 干型版本，殘糖 < 9 g/L，類似 Sherry 風格',                answer: 'Szamorodni Száraz', wrongs: ['Szamorodni Édes', 'Tokaji Aszú', 'Tokaji Furmint'] },
  { subject: '整串釀造的 Szamorodni 甜型版本，殘糖 ≥ 45 g/L',                                 answer: 'Szamorodni Édes',   wrongs: ['Szamorodni Száraz', 'Tokaji Aszú', '3 Puttonyos'] },
]
...
function nextQ() {
  ...
  if (qIdx.value >= questions.value.length - 1) {
    const _prev = parseInt(localStorage.getItem('hu_class_best') || '0')
    if (score.value > _prev) localStorage.setItem('hu_class_best', score.value)
    phase.value = 'final'
  } else {
...
async function loadLeaderboard(d) {
  ...
  try {
    const { data } = await supabase.from('quiz_scores')
      ...
      .eq('game_type', 'hungary_classification_quiz').eq('difficulty', d)
...
async function submitScore() {
  ...
  try {
    ...
    const { error } = await supabase.from('quiz_scores').insert({
      ...
      game_type: 'hungary_classification_quiz', difficulty: difficulty.value,
...
.cq-page {
  ...
  background: linear-gradient(160deg, #1a0f1a 0%, #261225 45%, #1f0d1c 100%);
  color: #f0f0f0;
...
.title { font-size: 1.8rem; font-weight: 800; color: #c084fc; margin: 0 0 8px; text-align: center; }
...
.easy-box { border-color: rgba(192,132,252,0.3); }
.easy-box .mode-name { color: #c084fc; }
...
.diff-list li::before { content: '· '; color: #c084fc; }
.diff-card.easy .diff-name { color: #c084fc; }
.diff-card.easy { border-color: rgba(192,132,252,0.3); }
...
.lb-title { font-size: 0.95rem; font-weight: 600; color: #c084fc; }
...
.lb-tabs button.active { background: rgba(192,132,252,0.2); border-color: #c084fc; color: #c084fc; }
...
.lb-table tr.mine td { background: rgba(192,132,252,0.08); }
.gold { color: #c084fc; font-weight: 700; }
...
.score-live { font-size: 1.1rem; font-weight: 700; color: #c084fc; }
...
.q-mode-badge.easy { background: rgba(192,132,252,0.15); color: #c084fc; border: 1px solid rgba(192,132,252,0.4); }
.q-mode-badge.hard { background: rgba(244,114,182,0.15); color: #f472b6; border: 1px solid rgba(244,114,182,0.4); }

.aging-desc { font-size: 0.98rem; color: #f0f0f0; line-height: 1.65; max-width: 420px; }
...
.opt-btn:hover:not(.frozen) { background: rgba(192,132,252,0.15); border-color: rgba(192,132,252,0.4); }
...
.final-score { font-size: 3rem; font-weight: 900; color: #c084fc; line-height: 1; }
...
.review-box { background: rgba(192,132,252,0.06); border: 1px solid rgba(192,132,252,0.2); border-radius: 12px; padding: 14px; margin-bottom: 20px; text-align: left; }
.review-title { font-size: 0.85rem; font-weight: 700; color: #c084fc; margin-bottom: 8px; }
...
.review-ans  { font-weight: 700; color: #c084fc; }

.btn-upload { width: 100%; padding: 13px; background: rgba(192,132,252,0.15); border: 1px solid rgba(192,132,252,0.4); color: #c084fc; border-radius: 12px; font-size: 1rem; font-weight: 700; cursor: pointer; margin-bottom: 10px; }
.btn-upload:hover:not(:disabled) { background: rgba(192,132,252,0.25); }
