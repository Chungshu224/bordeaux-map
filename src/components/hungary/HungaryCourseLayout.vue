<template>
  <div class="hungary-course-layout">
    <!-- 頂部導航欄 -->
    ...
    <!-- 底部抽屜（手機） -->
    <Teleport to="body">
      <div v-if="drawerOpen" class="hu-drawer-overlay" @click="drawerOpen = false"></div>
      <Transition name="hu-slide-up">
        <div v-if="drawerOpen" class="hu-chapter-drawer">
...
<style scoped>
/* ===========================
   CSS Variables (Hungary Red + Green)
   =========================== */
.hungary-course-layout {
  --primary: #c8102e;
  --primary-dark: #8c0b20;
  --primary-gradient: linear-gradient(135deg, #c8102e 0%, #8c0b20 100%);
  ...
  --shadow-md: 0 6px 24px rgba(200, 16, 46, 0.14);
  --radius-sm: 8px;
  --radius-md: 12px;
  --radius-lg: 16px;
  --radius-full: 9999px;
...
.module-icon-badge.completed { background: linear-gradient(135deg, var(--secondary) 0%, #2d4e38 100%); }
...
.lesson-card.completed { background: #f0faf3; border-color: var(--secondary); }
.lesson-num {
  width: 36px;
  height: 36px;
  border-radius: 50%;
...
.lesson-card.completed .lesson-num { background: linear-gradient(135deg, var(--secondary) 0%, #2d4e38 100%); }
...
.tag-done {
  background: linear-gradient(135deg, var(--secondary) 0%, #2d4e38 100%);
  color: white;
  padding: 5px 12px;
  border-radius: var(--radius-full);
  font-size: 0.75rem;
...
/* Transitions */
.hu-slide-up-enter-active, .hu-slide-up-leave-active { transition: transform 0.3s ease; }
.hu-slide-up-enter-from, .hu-slide-up-leave-to { transform: translateY(100%); }
