  ...
  <div ref="pageEl" class="cq-page" tabindex="-1" @keydown="onKey">
    ...
    <div v-if="phase === 'lobby'" class="lobby">
      <div class="lobby-inner">
        <h1 class="title">🥃 葡萄牙波特酒分級辨識</h1>
        <p class="subtitle">Port 6 種類別 + Tawny 陳年等級（10/20/30/40 yr）</p>
        ...
        <div class="mode-info">
          <div class="mode-box easy-box">
            ...
            <div>
              ...
              <div class="mode-detail">風味描述 → Port 類別</div>
            </div>
          </div>
          <div class="mode-box hard-box">
            ...
            <div>
              ...
              <div class="mode-detail">陳年描述 → Tawny 年份等級</div>
            </div>
          </div>
        </div>
        ...
        <div class="diff-cards">
          <div class="diff-card easy" @click="startGame('easy')">
            ...
            <div class="diff-desc">6 種波特酒類別</div>
            <ul class="diff-list">
              <li>4 個選項</li>
              <li>⏱ 每題 15 秒</li>
              <li>共 10 題</li>
          ...
          <div class="diff-card hard" @click="startGame('hard')">
            ...
            <div class="diff-desc">Tawny 陳年 + DOC</div>
            <ul class="diff-list">
              <li>4 個選項</li>
              <li>⏱ 每題 12 秒</li>
              <li>共 8 題</li>
...
const EASY_QUESTIONS = [
  { subject: '年輕鮮果風格，深紅紫色，未陳年（或極短桶陳），紅黑莓果風味',          answer: 'Ruby Port',   wrongs: ['Tawny Port', 'White Port', 'Vintage Port'] },
  { subject: '長時間桶陳氧化熟成，琥珀色，焦糖、核桃、無花果風味',                   answer: 'Tawny Port',  wrongs: ['Ruby Port', 'Colheita', 'White Port'] },
  { subject: 'Late Bottled Vintage，單一年份，桶陳 4-6 年後裝瓶，可即飲',            answer: 'LBV',         wrongs: ['Vintage Port', 'Tawny Port', 'Colheita'] },
  { subject: '最頂級單一年份波特，瓶中陳年數十年，需醒酒去除沉澱',                   answer: 'Vintage Port', wrongs: ['LBV', 'Tawny Port', 'Colheita'] },
  { subject: '單一年份的 Tawny，桶陳至少 7 年，標示年份不需 Vintage 宣告',           answer: 'Colheita',    wrongs: ['Vintage Port', 'Tawny Port', 'LBV'] },
  { subject: '白葡萄釀造的波特酒，從極干（Extra Seco）到甜（Doce）都有',             answer: 'White Port',  wrongs: ['Ruby Port', 'Tawny Port', 'Rosé Port'] },
  { subject: 'Port 入門款，多為混釀無年份，新鮮易飲',                                answer: 'Ruby Port',   wrongs: ['Tawny Port', 'LBV', 'Colheita'] },
  { subject: '優質 Vintage Port 的「平民版」，宣告年份但桶陳較久',                    answer: 'LBV',         wrongs: ['Vintage Port', 'Colheita', 'Tawny Port'] },
  { subject: '須由 IVDP 委員會宣告的特殊年份，每 10 年僅約 3 次',                     answer: 'Vintage Port', wrongs: ['LBV', 'Colheita', 'Tawny Port'] },
  { subject: '桶陳氧化色澤偏黃褐，常標示 10/20/30/40 年陳年',                        answer: 'Tawny Port',  wrongs: ['Ruby Port', 'Vintage Port', 'White Port'] },
]

// ── HARD: Tawny 陳年等級 + DOC 制度 ─────────────────────────────
const HARD_QUESTIONS = [
  { subject: '桶陳平均 10 年，蜂蜜杏仁風味，最入門的 Aged Tawny',                    answer: '10 Year Tawny', wrongs: ['20 Year Tawny', '30 Year Tawny', '40 Year Tawny'] },
  { subject: '桶陳平均 20 年，焦糖太妃糖，細膩平衡',                                  answer: '20 Year Tawny', wrongs: ['10 Year Tawny', '30 Year Tawny', '40 Year Tawny'] },
  { subject: '桶陳平均 30 年，蘭姆酒葡萄乾濃縮，深琥珀色',                            answer: '30 Year Tawny', wrongs: ['20 Year Tawny', '40 Year Tawny', 'Colheita'] },
  { subject: '桶陳平均 40 年以上，最稀有複雜，極致濃縮',                              answer: '40 Year Tawny', wrongs: ['30 Year Tawny', '20 Year Tawny', 'Colheita'] },
  { subject: '葡萄牙最高品質葡萄酒法定分級，相當於法國 AOC',                          answer: 'DOC',           wrongs: ['IPR', 'Vinho Regional', 'Vinho de Mesa'] },
  { subject: '次於 DOC 的法定分級，地理範圍較大，相當於法國 IGP',                      answer: 'Vinho Regional', wrongs: ['DOC', 'IPR', 'Vinho de Mesa'] },
  { subject: '葡萄牙最低等級的餐酒，無地理限制',                                      answer: 'Vinho de Mesa', wrongs: ['DOC', 'Vinho Regional', 'IPR'] },
  { subject: '杜羅河上游 Douro 地區的非加烈葡萄酒，使用相同葡萄品種',                  answer: 'Douro DOC',     wrongs: ['Porto DOC', 'Vinho Regional', 'Dão DOC'] },
]
...
function nextQ() {
  ...
  if (qIdx.value >= questions.value.length - 1) {
    const _prev = parseInt(localStorage.getItem('pt_class_best') || '0')
    if (score.value > _prev) localStorage.setItem('pt_class_best', score.value)
    phase.value = 'final'
  } else {
...
async function loadLeaderboard(d) {
  ...
  try {
    const { data } = await supabase.from('quiz_scores')
      ...
      .eq('game_type', 'portugal_classification_quiz').eq('difficulty', d)
...
async function submitScore() {
  ...
  try {
    ...
    const { error } = await supabase.from('quiz_scores').insert({
      ...
      game_type: 'portugal_classification_quiz', difficulty: difficulty.value,
...
.cq-page {
  ...
  background: linear-gradient(160deg, #1a0a0a 0%, #2a1418 45%, #1f0a14 100%);
  color: #f0f0f0;
...
.title { font-size: 1.8rem; font-weight: 800; color: #f97316; margin: 0 0 8px; text-align: center; }
...
.easy-box { border-color: rgba(249,115,22,0.3); }
.easy-box .mode-name { color: #f97316; }
...
.diff-list li::before { content: '· '; color: #f97316; }
.diff-card.easy .diff-name { color: #f97316; }
.diff-card.easy { border-color: rgba(249,115,22,0.3); }
...
.lb-title { font-size: 0.95rem; font-weight: 600; color: #f97316; }
...
.lb-tabs button.active { background: rgba(249,115,22,0.2); border-color: #f97316; color: #f97316; }
...
.lb-table tr.mine td { background: rgba(249,115,22,0.08); }
.gold { color: #f97316; font-weight: 700; }
...
.score-live { font-size: 1.1rem; font-weight: 700; color: #f97316; }
...
.q-mode-badge.easy { background: rgba(249,115,22,0.15); color: #f97316; border: 1px solid rgba(249,115,22,0.4); }
...
.opt-btn:hover:not(.frozen) { background: rgba(249,115,22,0.15); border-color: rgba(249,115,22,0.4); }
...
.final-score { font-size: 3rem; font-weight: 900; color: #f97316; line-height: 1; }
...
.review-box { background: rgba(249,115,22,0.06); border: 1px solid rgba(249,115,22,0.2); border-radius: 12px; padding: 14px; margin-bottom: 20px; text-align: left; }
.review-title { font-size: 0.85rem; font-weight: 700; color: #f97316; margin-bottom: 8px; }
...
.review-ans  { font-weight: 700; color: #f97316; }

.btn-upload { width: 100%; padding: 13px; background: rgba(249,115,22,0.15); border: 1px solid rgba(249,115,22,0.4); color: #f97316; border-radius: 12px; font-size: 1rem; font-weight: 700; cursor: pointer; margin-bottom: 10px; }
.btn-upload:hover:not(:disabled) { background: rgba(249,115,22,0.25); }
