  ...
  <div ref="pageEl" class="cq-page" tabindex="-1" @keydown="onKey">
    ...
    <div v-if="phase === 'lobby'" class="lobby">
      <div class="lobby-inner">
        <h1 class="title">🏷️ 德國分級辨識</h1>
        <p class="subtitle">Prädikat 等級 + 干甜分類（Trocken / Halbtrocken / Lieblich / Süss）</p>
        ...
        <div class="mode-info">
          <div class="mode-box easy-box">
            ...
            <div>
              ...
              <div class="mode-detail">採摘描述 → Prädikat 等級</div>
            </div>
          </div>
          <div class="mode-box hard-box">
            ...
            <div>
              ...
              <div class="mode-detail">殘糖描述 → 干甜等級</div>
            </div>
          </div>
        </div>
        ...
        <div class="diff-cards">
          <div class="diff-card easy" @click="startGame('easy')">
            ...
            <div class="diff-desc">Prädikat 6 等級辨識</div>
            <ul class="diff-list">
              <li>4 個選項</li>
              <li>⏱ 每題 15 秒</li>
              <li>共 10 題</li>
          ...
          <div class="diff-card hard" @click="startGame('hard')">
            ...
            <div class="diff-desc">干甜分類辨識</div>
            <ul class="diff-list">
              <li>4 個選項</li>
              <li>⏱ 每題 12 秒</li>
              <li>共 8 題</li>
    ...
    <div v-else-if="phase === 'playing' || phase === 'feedback'" class="game">
      ...
      <div class="question-card" :class="fbCardCls">
        <div class="q-mode-badge" :class="difficulty">
          {{ difficulty === 'easy' ? '🍇 Prädikat 等級' : '🍯 干甜分類' }}
        </div>
...
const EASY_QUESTIONS = [
  { subject: '基礎優質酒款，採摘成熟葡萄，Oechsle 約 70-85，多為輕盈干型',          answer: 'Kabinett',           wrongs: ['Spätlese', 'Auslese', 'Beerenauslese'] },
  { subject: '晚摘，葡萄較成熟，酒體較飽滿，可干可甜',                                  answer: 'Spätlese',           wrongs: ['Kabinett', 'Auslese', 'Beerenauslese'] },
  { subject: '逐串挑選最熟葡萄，常帶蜂蜜風味，可能受貴腐影響',                          answer: 'Auslese',            wrongs: ['Spätlese', 'Beerenauslese', 'Kabinett'] },
  { subject: '逐粒挑選感染貴腐的葡萄，極甜濃郁',                                        answer: 'Beerenauslese',      wrongs: ['Auslese', 'Trockenbeerenauslese', 'Eiswein'] },
  { subject: '逐粒挑選乾縮如葡萄乾的貴腐葡萄，世界最稀有的甜酒之一',                     answer: 'Trockenbeerenauslese', wrongs: ['Beerenauslese', 'Eiswein', 'Auslese'] },
  { subject: '採收凍結在藤上的葡萄，必須在 -7°C 以下採摘並壓榨，極甜濃縮',              answer: 'Eiswein',            wrongs: ['Trockenbeerenauslese', 'Beerenauslese', 'Auslese'] },
  { subject: 'QmP 入門等級，輕盈優雅，多搭配亞洲料理',                                  answer: 'Kabinett',           wrongs: ['Spätlese', 'Auslese', 'Trockenbeerenauslese'] },
  { subject: '酒精較高的晚摘酒，干型版本適合搭配豬肉',                                  answer: 'Spätlese',           wrongs: ['Kabinett', 'Auslese', 'Beerenauslese'] },
  { subject: '德文意為「精選」，葡萄手工逐串挑選',                                      answer: 'Auslese',            wrongs: ['Spätlese', 'Beerenauslese', 'Kabinett'] },
  { subject: '德文意為「冰酒」，從 1830 年代於德國誕生',                                 answer: 'Eiswein',            wrongs: ['Trockenbeerenauslese', 'Beerenauslese', 'Auslese'] },
]

// ── HARD: 干甜分類（殘糖等級）─────────────────────────────────────
const HARD_QUESTIONS = [
  { subject: '殘糖 ≤ 9 g/L（或最高 12 g/L 但酸度差需 ≤ 2 g/L），口感完全不甜',  answer: 'Trocken',     wrongs: ['Halbtrocken', 'Lieblich', 'Süss'] },
  { subject: '殘糖 9-18 g/L，輕微甜度，口感平衡',                                 answer: 'Halbtrocken', wrongs: ['Trocken', 'Lieblich', 'Süss'] },
  { subject: '殘糖 18-45 g/L，明顯甜度，但仍可佐餐',                              answer: 'Lieblich',    wrongs: ['Halbtrocken', 'Süss', 'Trocken'] },
  { subject: '殘糖 > 45 g/L，明顯甜酒風格',                                       answer: 'Süss',        wrongs: ['Lieblich', 'Halbtrocken', 'Trocken'] },
  { subject: '德文意為「干」，常見於 Riesling Trocken 標籤',                       answer: 'Trocken',     wrongs: ['Halbtrocken', 'Lieblich', 'Süss'] },
  { subject: '德文意為「半干」，介於干與甜之間，類似 off-dry',                      answer: 'Halbtrocken', wrongs: ['Trocken', 'Lieblich', 'Süss'] },
  { subject: '德文意為「可愛、討喜」，意指明顯甜度的中等甜酒',                      answer: 'Lieblich',    wrongs: ['Halbtrocken', 'Süss', 'Trocken'] },
  { subject: '德文意為「甜」，殘糖最高的等級',                                     answer: 'Süss',        wrongs: ['Lieblich', 'Halbtrocken', 'Trocken'] },
]
...
function nextQ() {
  ...
  if (qIdx.value >= questions.value.length - 1) {
    const _prev = parseInt(localStorage.getItem('de_class_best') || '0')
    if (score.value > _prev) localStorage.setItem('de_class_best', score.value)
    phase.value = 'final'
  } else {
...
async function loadLeaderboard(d) {
  ...
  try {
    const { data } = await supabase.from('quiz_scores')
      ...
      .eq('game_type', 'germany_classification_quiz').eq('difficulty', d)
...
async function submitScore() {
  ...
  try {
    ...
    const { error } = await supabase.from('quiz_scores').insert({
      ...
      game_type: 'germany_classification_quiz', difficulty: difficulty.value,
...
.cq-page {
  ...
  background: linear-gradient(160deg, #0a1a14 0%, #1a2a1f 45%, #14231a 100%);
  color: #f0f0f0;
...
.title { font-size: 1.8rem; font-weight: 800; color: #facc15; margin: 0 0 8px; text-align: center; }
...
.easy-box { border-color: rgba(250,204,21,0.3); }
.easy-box .mode-name { color: #facc15; }
...
.diff-list li::before { content: '· '; color: #facc15; }
.diff-card.easy .diff-name { color: #facc15; }
.diff-card.easy { border-color: rgba(250,204,21,0.3); }
...
.lb-title { font-size: 0.95rem; font-weight: 600; color: #facc15; }
...
.lb-tabs button.active { background: rgba(250,204,21,0.2); border-color: #facc15; color: #facc15; }
...
.lb-table tr.mine td { background: rgba(250,204,21,0.08); }
...
.score-live { font-size: 1.1rem; font-weight: 700; color: #facc15; }
...
.q-mode-badge.easy { background: rgba(250,204,21,0.15); color: #facc15; border: 1px solid rgba(250,204,21,0.4); }
...
.opt-btn:hover:not(.frozen) { background: rgba(250,204,21,0.15); border-color: rgba(250,204,21,0.4); }
...
.final-score { font-size: 3rem; font-weight: 900; color: #facc15; line-height: 1; }
...
.review-box { background: rgba(250,204,21,0.06); border: 1px solid rgba(250,204,21,0.2); border-radius: 12px; padding: 14px; margin-bottom: 20px; text-align: left; }
.review-title { font-size: 0.85rem; font-weight: 700; color: #facc15; margin-bottom: 8px; }
...
.review-ans  { font-weight: 700; color: #facc15; }

.btn-upload { width: 100%; padding: 13px; background: rgba(250,204,21,0.15); border: 1px solid rgba(250,204,21,0.4); color: #facc15; border-radius: 12px; font-size: 1rem; font-weight: 700; cursor: pointer; margin-bottom: 10px; }
.btn-upload:hover:not(:disabled) { background: rgba(250,204,21,0.25); }
