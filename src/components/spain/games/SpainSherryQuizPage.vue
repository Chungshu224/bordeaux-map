<template>
  <div ref="pageEl" class="ssq-page" tabindex="-1" @keydown="onKey">
    <button class="back-btn" @click="handleBack">← 返回</button>

    <!-- ══ LOBBY ══ -->
    <div v-if="phase === 'lobby'" class="lobby">
      <div class="lobby-inner">
        <h1 class="title">🏺 雪莉酒識別賽</h1>
        <p class="subtitle">閱讀描述，辨識 7 種西班牙雪莉酒類型</p>

        <div class="sherry-types">
          <span v-for="t in SHERRY_TYPES" :key="t.name" class="type-chip" :style="{ background: t.bg, color: t.fg, borderColor: t.border }">{{ t.name }}</span>
        </div>

        <div class="diff-cards">
          <div class="diff-card easy" @click="startGame('easy')">
            <div class="diff-icon">🍶</div>
            <div class="diff-name">標準</div>
            <div class="diff-desc">7 種核心類型</div>
            <ul class="diff-list">
              <li>4 個選項</li>
              <li>⏱ 每題 20 秒</li>
              <li>共 7 題全覆蓋</li>
          ...
          <div class="diff-card hard" @click="startGame('hard')">
            ...
            <div class="diff-desc">10 種含 Pale Cream / Moscatel</div>
            <ul class="diff-list">
              <li>5 個選項</li>
              <li>⏱ 每題 15 秒</li>
              <li>共 10 題</li>
        ...
        <div class="lb-box">
          <div class="lb-head">
            ...
            <div class="lb-tabs">
              <button :class="{ active: lbTab === 'easy' }" @click="setLbTab('easy')">標準</button>
    ...
    <div v-else-if="phase === 'playing' || phase === 'feedback'" class="game">
      ...
      <div class="question-card" :class="fbCardCls">
        <div class="sherry-label">雪莉酒類型識別</div>
        <div class="sherry-desc">{{ currentQ?.desc }}</div>
        <div v-if="currentQ?.hint" class="sherry-hint">💡 {{ currentQ.hint }}</div>
      ...
      <div class="options-grid" :class="`cols-${optCols}`">
        <button
    ...
    <div v-else-if="phase === 'final'" class="final">
      <div class="final-card">
        ...
        <div class="final-chips">
          ...
          <span class="chip">{{ difficulty === 'easy' ? '🍶 標準' : '🔥 進階' }}</span>
        </div>
        ...
        <div v-if="wrongItems.length" class="review-box">
          ...
          <div v-for="w in wrongItems" :key="w.name" class="review-row">
            <span class="review-name review-desc-text">{{ w.name }}</span>
...
// ── Sherry type display info ───────────────────────────────────
const SHERRY_TYPES = [
  { name: 'Fino',          bg: 'rgba(253,224,71,0.15)',  fg: '#fde047', border: 'rgba(253,224,71,0.4)' },
  { name: 'Manzanilla',    bg: 'rgba(74,222,128,0.15)',  fg: '#4ade80', border: 'rgba(74,222,128,0.4)' },
  { name: 'Amontillado',   bg: 'rgba(251,146,60,0.15)',  fg: '#fb923c', border: 'rgba(251,146,60,0.4)' },
  { name: 'Palo Cortado',  bg: 'rgba(167,139,250,0.15)', fg: '#a78bfa', border: 'rgba(167,139,250,0.4)' },
  { name: 'Oloroso',       bg: 'rgba(248,113,113,0.15)', fg: '#f87171', border: 'rgba(248,113,113,0.4)' },
  { name: 'Pedro Ximénez', bg: 'rgba(100,60,30,0.4)',    fg: '#d4a574', border: 'rgba(200,130,60,0.4)' },
  { name: 'Cream',         bg: 'rgba(200,169,110,0.15)', fg: '#c8a96e', border: 'rgba(200,169,110,0.4)' },
]

// ── Question banks ─────────────────────────────────────────────
const EASY_QUESTIONS = [
  {
    desc: '受 flor 酵母膜保護，極乾型，淡金色，帶礦物感與鹽漬堅果香，酒精約 15–18%',
    answer: 'Fino',
    wrongs: ['Manzanilla','Amontillado','Oloroso','Cream','Pedro Ximénez']
  },
  {
    desc: '產自桑盧卡爾德巴拉梅達（Sanlúcar de Barrameda），比 Fino 更輕盈，帶海風鹽霧，必須在當地陳釀',
    answer: 'Manzanilla',
    wrongs: ['Fino','Amontillado','Palo Cortado','Oloroso','Cream']
  },
  {
    desc: '由 Fino 失去 flor 後繼續氧化陳年而成，榛果焦糖，中等琥珀色，乾至半乾，風味複雜',
    answer: 'Amontillado',
    wrongs: ['Fino','Palo Cortado','Oloroso','Cream','Manzanilla']
  },
  {
    desc: '偶然失去 flor 的極稀有類型，兼具 Amontillado 細膩與 Oloroso 豐厚，帶橘皮核桃蜜餞香',
    answer: 'Palo Cortado',
    wrongs: ['Amontillado','Oloroso','Fino','Cream','Manzanilla']
  },
  {
    desc: '完全氧化陳年，無 flor 保護，深琥珀色，堅果乾果太妃糖，乾型飽滿，風味濃縮',
    answer: 'Oloroso',
    wrongs: ['Amontillado','Palo Cortado','Cream','Pedro Ximénez','Manzanilla']
  },
  {
    desc: '以 100% 風乾 Pedro Ximénez 葡萄釀製，濃稠近黑色糖漿，極甜，葡萄乾巧克力可可',
    answer: 'Pedro Ximénez',
    wrongs: ['Cream','Oloroso','Moscatel','Amontillado','Palo Cortado']
  },
  {
    desc: '以 Oloroso 或 Amontillado 為基底加甜，深棕色，甜潤易飲，英國市場最暢銷雪莉類型',
    answer: 'Cream',
    wrongs: ['Pedro Ximénez','Oloroso','Amontillado','Pale Cream','Manzanilla']
  },
]
...
const HARD_QUESTIONS = [
  ...EASY_QUESTIONS,
  {
    desc: '以 Fino 或 Manzanilla 為基底加甜，但顏色依然淺金色，甜潤但較 Cream 輕盈，帶花香',
    answer: 'Pale Cream',
    hint: '外觀淺色，與一般 Cream 的深色形成對比',
    wrongs: ['Cream','Fino','Manzanilla','Amontillado','Medium']
  },
  {
    desc: '介於乾型與甜型之間，以 Amontillado 為基礎，半乾口感，帶焦糖榛果，甜度居中',
    answer: 'Medium',
    wrongs: ['Amontillado','Cream','Pale Cream','Fino','Oloroso']
  },
  {
    desc: '以 Moscatel 葡萄釀製，類似 PX 但帶橙花玫瑰香氣，甜潤，比 PX 顏色稍淺',
    answer: 'Moscatel Sherry',
    wrongs: ['Pedro Ximénez','Cream','Pale Cream','Oloroso','Amontillado']
  },
]
...
const optCols     = computed(() => difficulty.value === 'easy' ? 4 : 5)
const comboMult   = computed(() => streak.value >= 7 ? 3 : streak.value >= 4 ? 2 : streak.value >= 3 ? 1.5 : 1)
const timerColor  = computed(() => timerPct.value > 60 ? '#fbbf24' : timerPct.value > 25 ? '#fb923c' : '#ef4444')
...
function answer(opt) {
  ...
  if (correct) {
    ...
    streak.value = 0
    // Store a trimmed version of the description for review
    const shortDesc = currentQ.value.desc.length > 40
      ? currentQ.value.desc.slice(0, 40) + '…'
      : currentQ.value.desc
    wrongItems.value.push({ name: shortDesc, answer: currentQ.value.answer })
  }
  phase.value = 'feedback'
  feedTimer = setTimeout(nextQ, 1500)
}
...
function nextQ() {
  ...
  if (qIdx.value >= questions.value.length - 1) {
    const _prev = parseInt(localStorage.getItem('sp_sherry_best') || '0')
    if (score.value > _prev) localStorage.setItem('sp_sherry_best', score.value)
    phase.value = 'final'
  } else {
    qIdx.value++; phase.value = 'playing'
    startTimer(difficulty.value === 'easy' ? 20 : 15)
  }
}
...
async function loadLeaderboard(d) {
  ...
  try {
    const { data } = await supabase.from('quiz_scores')
      ...
      .eq('game_type', 'spain_sherry_quiz').eq('difficulty', d)
...
async function submitScore() {
  ...
  try {
    ...
    const { error } = await supabase.from('quiz_scores').insert({
      ...
      game_type: 'spain_sherry_quiz', difficulty: difficulty.value,
...
.lobby-inner { width: 100%; max-width: 620px; }
.title { font-size: 1.8rem; font-weight: 800; color: #fbbf24; margin: 0 0 8px; text-align: center; }
.subtitle { color: rgba(255,255,255,0.55); margin: 0 0 16px; text-align: center; font-size: 0.95rem; }

.sherry-types { display: flex; flex-wrap: wrap; gap: 6px; justify-content: center; margin-bottom: 24px; }
.type-chip { font-size: 0.78rem; font-weight: 700; padding: 4px 11px; border-radius: 20px; border: 1px solid transparent; }
...
.diff-list li::before { content: '· '; color: #fbbf24; }
.diff-card.easy .diff-name { color: #fbbf24; }
.diff-card.easy { border-color: rgba(251,191,36,0.3); }
.diff-card.hard .diff-name { color: #f87171; }
.diff-card.hard { border-color: rgba(248,113,113,0.3); }
...
.lb-title { font-size: 0.95rem; font-weight: 600; color: #fbbf24; }
...
.lb-tabs button.active { background: rgba(251,191,36,0.2); border-color: #fbbf24; color: #fbbf24; }
...
.lb-table tr.mine td { background: rgba(251,191,36,0.08); }
...
.score-live { font-size: 1.1rem; font-weight: 700; color: #fbbf24; }
...
.question-card {
  flex: 1; min-height: 0; margin: 4px 20px 12px; overflow-y: auto;
...
.sherry-desc { font-size: 1rem; color: #f0f0f0; line-height: 1.65; max-width: 420px; margin-bottom: 8px; }
.sherry-hint { font-size: 0.82rem; color: rgba(255,255,255,0.4); margin-top: 4px; }
.feedback-row { margin-top: 12px; font-size: 1rem; font-weight: 700; }
.fb-ok    { color: #4ade80; }
.fb-tmout { color: #fbbf24; }
.fb-wrong { color: #f87171; }

.options-grid { display: grid; gap: 8px; padding: 0 20px 20px; flex-shrink: 0; }
.options-grid.cols-4 { grid-template-columns: 1fr 1fr; }
.options-grid.cols-5 { grid-template-columns: 1fr 1fr 1fr; }

.opt-btn {
  padding: 12px 8px; background: rgba(255,255,255,0.07); border: 1px solid rgba(255,255,255,0.12);
  border-radius: 12px; color: #f0f0f0; font-size: 0.88rem; font-weight: 600;
  cursor: pointer; transition: all 0.15s;
}
.opt-btn:hover:not(.frozen) { background: rgba(251,191,36,0.15); border-color: rgba(251,191,36,0.4); }
...
.final-score { font-size: 3rem; font-weight: 900; color: #fbbf24; line-height: 1; }
...
.review-box { background: rgba(251,191,36,0.06); border: 1px solid rgba(251,191,36,0.2); border-radius: 12px; padding: 14px; margin-bottom: 20px; text-align: left; }
.review-title { font-size: 0.85rem; font-weight: 700; color: #fbbf24; margin-bottom: 8px; }
.review-row { display: flex; align-items: flex-start; gap: 8px; padding: 5px 0; font-size: 0.8rem; border-bottom: 1px solid rgba(255,255,255,0.05); }
.review-row:last-child { border-bottom: none; }
.review-desc-text { flex: 1; color: rgba(255,255,255,0.6); font-size: 0.78rem; line-height: 1.4; }
.review-sep  { color: rgba(255,255,255,0.3); flex-shrink: 0; }
.review-ans  { color: #fbbf24; font-weight: 700; flex-shrink: 0; }

.btn-upload { width: 100%; padding: 13px; background: rgba(251,191,36,0.15); border: 1px solid rgba(251,191,36,0.4); color: #fbbf24; border-radius: 12px; font-size: 1rem; font-weight: 700; cursor: pointer; margin-bottom: 10px; }
...
@media (max-width: 480px) {
  ...
  .options-grid.cols-5 { grid-template-columns: 1fr 1fr; }
  .sherry-desc { font-size: 0.9rem; }
}
</style>
