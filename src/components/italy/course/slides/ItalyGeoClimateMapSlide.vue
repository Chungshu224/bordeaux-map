<template>
  <div class="geo-slide">
    ...
    <div class="slide-top">
      ...
      <div class="layer-tabs">
        <button
          ...
          @click="activeLayer = lay.key; activeZone = null"
        >
          {{ lay.icon }} {{ lay.label }}
        </button>
      </div>
    ...
    <div class="slide-body">
      <!-- Italy SVG Map -->
      <div class="map-panel">
        <svg viewBox="0 0 210 440" class="italy-svg" preserveAspectRatio="xMidYMid meet">
          <!-- Sardinia -->
          <ellipse
            cx="14" cy="142" rx="12" ry="34"
            :fill="zoneColor('islands')" stroke="white" stroke-width="1.5"
            class="zone-shape" :class="{ 'zone-active': activeZone === 'islands' }"
            @click="activeZone = 'islands'"
          />
          <text x="14" y="145" text-anchor="middle" class="zone-mini-label">薩丁</text>

          <!-- North Italy -->
          <polygon
            points="32,8 168,8 175,42 168,65 148,77 100,84 52,77 32,65 25,42"
            :fill="zoneColor('north')" stroke="white" stroke-width="1.5"
            class="zone-shape" :class="{ 'zone-active': activeZone === 'north' }"
            @click="activeZone = 'north'"
          />
          <text x="100" y="46" text-anchor="middle" class="zone-label">NORD</text>
          <text x="100" y="59" text-anchor="middle" class="zone-sublabel">北部</text>

          <!-- Center Italy -->
          <polygon
            points="52,77 148,77 155,132 153,184 142,217 100,226 58,217 47,184 45,132"
            :fill="zoneColor('center')" stroke="white" stroke-width="1.5"
            class="zone-shape" :class="{ 'zone-active': activeZone === 'center' }"
            @click="activeZone = 'center'"
          />
          <text x="100" y="148" text-anchor="middle" class="zone-label">CENTRO</text>
          <text x="100" y="161" text-anchor="middle" class="zone-sublabel">中部</text>

          <!-- South Italy (boot shape) -->
          <path
            d="M58,217 L142,217 L150,257 L158,296 L164,330 L164,352
               L176,362 L174,375 L163,382 L152,374
               L148,390 L100,400 L78,387 L68,380 L52,374 L38,352
               L36,330 L42,296 L50,257 Z"
            :fill="zoneColor('south')" stroke="white" stroke-width="1.5"
            class="zone-shape" :class="{ 'zone-active': activeZone === 'south' }"
            @click="activeZone = 'south'"
          />
          <text x="96" y="306" text-anchor="middle" class="zone-label">SUD</text>
          <text x="96" y="319" text-anchor="middle" class="zone-sublabel">南部</text>

          <!-- Sicily -->
          <ellipse
            cx="80" cy="420" rx="52" ry="15"
            :fill="zoneColor('islands')" stroke="white" stroke-width="1.5"
            class="zone-shape" :class="{ 'zone-active': activeZone === 'islands' }"
            @click="activeZone = 'islands'"
          />
          <text x="80" y="423" text-anchor="middle" class="zone-mini-label">西西里</text>
        </svg>
        ...
        <div class="map-legend">
          <div
            ...
            @click="activeZone = activeZone === z.id ? null : z.id"
          >
...
const LAYERS = [
  {
    ...
    zones: {
      north:   { icon:'🏔️', color:'#4CAF50', darkColor:'#2e6b3e',
      ...
      center:  { icon:'🌿', color:'#8BC34A', darkColor:'#558b2f',
      ...
      south:   { icon:'☀️', color:'#FF9800', darkColor:'#e65100',
      ...
      islands: { icon:'🏝️', color:'#F44336', darkColor:'#b71c1c',
  ...
  },
  {
    ...
    zones: {
      north:   { icon:'❄️', color:'#42a5f5', darkColor:'#1565C0',
      ...
      center:  { icon:'🌤️', color:'#66BB6A', darkColor:'#2e7d32',
      ...
      south:   { icon:'🌞', color:'#FFA726', darkColor:'#e65100',
      ...
      islands: { icon:'🔥', color:'#EF5350', darkColor:'#b71c1c',
  ...
  },
  {
    ...
    zones: {
      north:   { icon:'🪨', color:'#8D6E63', darkColor:'#6D4C41',
      ...
      center:  { icon:'🏺', color:'#A1887F', darkColor:'#795548',
      ...
      south:   { icon:'🌋', color:'#BCAAA4', darkColor:'#8d6e63',
      ...
      islands: { icon:'⚫', color:'#D7CCC8', darkColor:'#795548',
...
.zone-shape:hover { opacity: 0.85; filter: brightness(1.08); }
.zone-active { stroke: #fff !important; stroke-width: 3 !important; filter: drop-shadow(0 0 6px rgba(0,0,0,0.4)); }
.zone-label {
  font-size: 9px;
  font-weight: 800;
  fill: white;
...
.legend-item:hover, .legend-item.active { background: rgba(123,31,42,0.07); font-weight: 600; }
.legend-dot {
