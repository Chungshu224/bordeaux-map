...
async function loadLeaderboard(diff) {
  ...
  try {
    const { data } = await supabase.from('game_scores')
      ...
      .eq('game_type', 'au_state_quiz').eq('difficulty', diff)
...
async function submitScore() {
  ...
  try {
    const { error } = await supabase.from('game_scores').insert({
      game_type: 'au_state_quiz', difficulty: difficulty.value,
...
.score-td { color: #38bdf8; font-weight: 700; }
.date-td { color: rgba(255,255,255,0.3); }
.btn-upload { width: 100%; padding: 12px; background: linear-gradient(135deg, #0284c7, #38bdf8); border: none; border-radius: 12px; color: #fff; font-weight: 700; cursor: pointer; margin-top: 12px; transition: opacity 0.2s; }
