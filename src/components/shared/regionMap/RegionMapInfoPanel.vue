    ...
    <div class="rmap-info-header" :style="headerStyle">
      ...
      <div class="rmap-action-buttons">
        ...
        <button class="rmap-action-btn" @click="$emit('toggle-collapse')" :title="collapsed ? '展開資訊' : '收合資訊'">
          <span class="rmap-btn-icon" :style="{ display:'inline-block', transform: collapsed ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.3s' }">▾</span>
        </button>
        <!-- 發音 -->
        <button class="rmap-action-btn audio" @click="$emit('play-audio')"
          ...
          <span class="rmap-btn-icon">{{ !audioAvailable ? '🔇' : isPlayingAudio ? '🎵' : '🔊' }}</span>
        </button>
        <!-- 重置視角 -->
      ...
      <div v-show="!collapsed" class="rmap-info-content-wrapper">
        <div class="rmap-info-content">
          ...
          <div v-if="info?.styles?.length" class="rmap-section">
            <div class="rmap-section-title">🍷 {{ t('bordeaux.map.info.styles') }}</div>
          ...
          <div v-if="info?.grapes?.length" class="rmap-section">
            <div class="rmap-section-title">🍇 {{ t('bordeaux.map.info.grapes') }}</div>
            <div class="rmap-tag-list">
          ...
          <div v-if="info?.estates?.length" class="rmap-section">
            <div class="rmap-section-title">🏆 {{ t('bordeaux.map.info.estates') }}</div>
...
function styleBadgeColor(style) {
  const map = {
    ...
    'Red Wine': { bg: '#8B0000', color: '#fff' },
    '白酒': { bg: '#C8A44E', color: '#333' },
    'Dry White': { bg: '#C8A44E', color: '#333' },
    '粉紅酒': { bg: '#E8A4B0', color: '#5a2533' },
    'Rosé': { bg: '#E8A4B0', color: '#5a2533' },
    '玫瑰紅': { bg: '#E8A4B0', color: '#5a2533' },
    '甜酒': { bg: '#F4D35E', color: '#5a3a00' },
    'Sweet Wine': { bg: '#F4D35E', color: '#5a3a00' },
    '貴腐酒': { bg: '#C9A227', color: '#fff' },
    '氣泡酒': { bg: '#B8E0D2', color: '#1a4d3a' },
    'Sparkling': { bg: '#B8E0D2', color: '#1a4d3a' },
    '加烈酒': { bg: '#5C2A2A', color: '#fff' },
    '赤ワイン': { bg: '#8B0000', color: '#fff' },
    '白ワイン': { bg: '#C8A44E', color: '#333' },
    'ロゼ':     { bg: '#E8A4B0', color: '#5a2533' },
    '甘口':     { bg: '#F4D35E', color: '#5a3a00' },
    'スパークリング': { bg: '#B8E0D2', color: '#1a4d3a' }
  }
...
@media (max-width: 768px) {
  ...
  .rmap-action-btn { width: 38px; height: 38px; }
  .rmap-info-content-wrapper {
    max-height: 55vh;
  }
  .rmap-info-content {
