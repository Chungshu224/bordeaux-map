#!/usr/bin/env node
/**
 * scripts/build-italy-climate.mjs
 *
 * Build Italy climate dataset for map heatmap layer.
 * Data source: Open-Meteo archive daily API (no key required).
 *
 * Output:
 *   public/data/italy-climate.json
 *
 * Usage:
 *   node scripts/build-italy-climate.mjs
 *   node scripts/build-italy-climate.mjs --dry-run
...
const italyRegionsDir = join(rootDir, 'public', 'italy', 'regions')
const outputPath = join(rootDir, 'public', 'data', 'italy-climate.json')
...
function findRegionJsonFiles() {
  if (!existsSync(italyRegionsDir)) return []
  const allFiles = walkDirs(italyRegionsDir)
  return allFiles.filter((f) => f.endsWith('-regions.json'))
}
...
const dirFileIndex = new Map()
function getDirectoryFileIndex(dirPath) {
  if (dirFileIndex.has(dirPath)) return dirFileIndex.get(dirPath)
  const map = new Map()
  if (existsSync(dirPath)) {
    for (const name of readdirSync(dirPath)) {
      if (!name.endsWith('.geojson')) continue
      map.set(normalizeName(name.replace(/\.geojson$/i, '')), name)
    }
  }
  dirFileIndex.set(dirPath, map)
  return map
}

function resolveGeojsonPath(regionFolder, item) {
  const type = item.type === 'DOCG' ? 'DOCG' : item.type === 'IGT' ? 'IGT' : 'DOC'
  const baseDir = join(italyRegionsDir, regionFolder, 'geojson', type)

  const idStem = String(item.id || '').replace(/^(DOCG|DOC|IGT)\//, '')
  const directById = join(baseDir, `${idStem}.geojson`)
  if (existsSync(directById)) return directById

  const directByName = join(baseDir, `${item.name} ${item.type}.geojson`)
  if (existsSync(directByName)) return directByName

  const index = getDirectoryFileIndex(baseDir)
  const candidates = [
    normalizeName(idStem),
    normalizeName(item.id),
    normalizeName(item.name),
    normalizeName(`${item.name} ${item.type}`)
  ]

  for (const key of candidates) {
    const filename = index.get(key)
    if (filename) return join(baseDir, filename)
  }
...
function computeCentroid(filePath) {
  ...
  const crsNameRaw =
    ...
    geojson?.features?.[0]?.crs?.properties?.name ||
    ''
  const crsName = String(crsNameRaw).toUpperCase()
  ...
  const projectPoint = (x, y) => {
    ...
    const use3003 = crsName.includes('3003') || (looksProjected && x < 2100000)
    const use3004 = crsName.includes('3004') || (looksProjected && x >= 2100000)

    if (use3003 || use3004) {
      try {
        const epsg = use3004 ? 'EPSG:3004' : 'EPSG:3003'
        ;[lng, lat] = proj4(epsg, 'WGS84', [x, y])
      } catch {
        return null
      }
    }
  ...
  const visit = (coords) => {
    ...
    if (typeof coords[0] === 'number' && typeof coords[1] === 'number') {
      const p = projectPoint(coords[0], coords[1])
      if (!p) return
      sumLon += p[0]
      sumLat += p[1]
      count += 1
...
async function main() {
  console.log('Italy climate build start')
  console.log(`years: ${START_YEAR}-${END_YEAR}, dry-run: ${DRY_RUN ? 'yes' : 'no'}`)

  const regionFiles = findRegionJsonFiles()
  if (!regionFiles.length) {
    console.error('No italy region json files found.')
    process.exit(1)
  }

  const tasks = []
  for (const regionFile of regionFiles) {
    const regionFolder = dirname(regionFile).split(/[\\/]/).pop()
    if (REGION_FILTER && regionFolder !== REGION_FILTER) continue

    const raw = JSON.parse(readFileSync(regionFile, 'utf-8'))
    for (const item of raw) {
      if (!['DOC', 'DOCG', 'IGT'].includes(item.type)) continue
      const geoPath = resolveGeojsonPath(regionFolder, item)
      tasks.push({ regionFolder, item, geoPath })
    }
  }
  ...
  if (!tasks.length) {
    console.error('No AOC tasks found with current filter.')
    process.exit(1)
  }
  ...
  if (DRY_RUN) {
    const preview = work.slice(0, 20).map((w) => ({
      id: w.item.id,
      region: w.regionFolder,
      centroid: w.centroid,
      file: w.geoPath.split(/[\\/]/).slice(-3).join('/')
    }))
